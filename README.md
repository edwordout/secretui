...
Pages flow `Collections → Items → Details`. `↑/↓` or `j/k` move, `Enter`/`→`/`l`/`Tab` go forward, `Esc`/`←`/`h` go back, `/` searches inline, and `?` shows help. On Collections, `n` creates a New Collection; on Items, `n` creates a New Item. On Details, use `↑/↓` to scroll, `←/→` to choose an action button, and `Enter` to activate it.
